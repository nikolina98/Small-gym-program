...
//
	 ...
	 * Create the dialog.
	 */
	public ListUsers(Crud crud) {
		this.crud = crud;
		setTitle("Spisak aktivnih clanarina");
		...
		List<User> users = crud.getActiveUsers();
		...
		JButton btnMore = new JButton("Produzi clanarinu");
		btnMore.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(user.getStartDate().compareTo(now) <= 0 && user.getEndDate().compareTo(now) <= 0) {
					user.setMemberStatus(false);
					crud.switchUserReverse(user);
				}
		...
		JButton cancel = new JButton("Otkazi clanarinu");
		cancel.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int maxS = lsm.getMaxSelectionIndex();
				if(maxS == -1)
					return;
				User u = tm.get(maxS);
				
				LocalDate ld = LocalDate.now().minusDays(1);
				u.setMemberStatus(false);
				u.setStartDate(ld);
				u.setEndDate(ld);
				getCrud().switchUserReverse(u);
				tm.fireTableDataChanged();
			}
		});
		cancel.setBounds(780, 450, 230, 50);
		cancel.setForeground(new Color(25, 25, 112));
		cancel.setBackground(Color.WHITE);
		cancel.setFont(new Font("Candara", Font.BOLD, 20));
